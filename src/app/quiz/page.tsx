...
export default function Quiz() {
    ...
    const handleClick = async(e: MouseEvent) => {
        ...
        if(skill === "Reading"){
            const { data } = await axios.get("http://localhost:8080/api/v1/reading");
            navigate(`/quiz/reading/${data.id}`);
        }
        else{
            const { data } = await axios.get("http://localhost:8080/api/v1/writing");
            navigate(`/quiz/writing/${data.id}`);
        }
